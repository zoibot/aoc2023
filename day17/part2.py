...
while len(q) > 0:
    ...
    for d in dirs:
        ...
        if straights < 3 and d != lastd and lastd != (0,0):
            #print('uh', straights, d, lastd)
            continue
        if newstraights >= 10:
            continue
        ...
        if len(grid[0])-1 == xnew and len(grid)-1 == ynew:
            if newheat < best_so_far and straights > 3:
                best_so_far = newheat
        ...
        prio = newheat + (len(grid[0])-1-x) + (len(grid)-1-y) 
        heappush(q, (prio, xnew, ynew, newheat, d, newstraights, path+((xnew,ynew,d),)))
